...
When invoked:
0. **Invoke `rule-audit` skill (Phase 2 mode)** to load project + global rule
   checklist scoped to TypeScript / React files. Every finding below must
   either map to a rule from that checklist or be a CRITICAL/HIGH severity
   issue (security, correctness, data loss). Cite the broken rule by name
