...
- Preserve a leading ticket key like `AP-287` when present.
- Convert the remaining hyphenated slug into readable words.
- Prefer `TICKET: human readable summary` when a ticket key exists.
- Example: `AP-287-older-plan-loading-during-new-plan-generation` → `AP-287: Older plan loading during new plan generation`.
